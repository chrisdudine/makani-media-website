...
async function selectDay(day) {
  ...
  try {
    ...
    renderTimes(Object.fromEntries(data.slots.map((slot) => [slot.time, slot.available === true])));
  } catch (error) {
    if (selectedDate === requestedDate)
      timeSlots.innerHTML = '<span class="hint">Availability is temporarily unavailable. Please try again shortly.</span>';
  }
}
