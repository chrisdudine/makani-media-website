...
async function getAvailability(env, date, dayCount = 1) {
  ...
  const dayEnd = addMinutes(dayStart, 24 * 60 * dayCount);
  ...
  if (env.GOOGLE_CALENDAR_ENABLED === "true") {
    ...
    blocks.push(...(await googleBusy(env, dayStart, dayEnd)));
  }
  const days = {};
  for (let offset = 0; offset < dayCount; offset++) {
    const key = addMinutes(new Date(`${date}T00:00:00Z`), offset * 1440)
      .toISOString()
      .slice(0, 10);
    days[key] = slots.map((slot) => {
      const start = hawaiiDateTime(key, slot.time),
      ...
      return {
        ...
        available:
          key > "2026-12-15" &&
          start > new Date() &&
          !blocks.some(([a, b]) => overlaps(start, end, a, b)),
      };
    });
  }
  return dayCount === 1 ? days[date] : days;
}
    ...
    if (request.method === "GET" && url.pathname === "/api/availability") {
      ...
      if (month) {
        if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(month))
          return json({ error: "A valid month is required." }, 400);
        try {
          ...
          const days = await getAvailability(env, `${month}-01`, count);
          return json({ month, timezone: "Pacific/Honolulu", days }, 200, {
      ...
      try {
        ...
        return json({ date, timezone: "Pacific/Honolulu", slots }, 200, {
          "Cache-Control": "no-store",
        });
      } catch (error) {
